  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const quote = await createQuote({
        ...
        plateId: customPlate ? null : (isMultiProduct ? (parseInt(products[0].selectedPlateId) || null) : parseInt(ctx.selectedPlateId)),
        ...
        products: isMultiProduct ? products.map((p, i) => ({
          ...
          varnishMlPerPlate: p.varnishMlPerPlate,
        ...
        amalgameRuns: (isMultiProduct && amalgameGroups.length > 0) ? amalgameGroups.map((g, i) => ({
          ...
          varnishMlPerPlate: g.varnishMlPerPlate,
