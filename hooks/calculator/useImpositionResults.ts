...
function computeGroupResult(
  ...
  const groupCosts = calculateCosts({
    ...
    varnishMlPerPlate: group.varnishMlPerPlate,
    ...
    assemblyTimePerPieceSeconds: 0, packTimePerPieceSeconds: 0, hasAssemblyNotice: false, hasPoseEtiquette: false,
  ...
  return useMemo(() => {
    ...
    const productSlotResults: ProductSlotResult[] = products.map((slot) => {
      ...
      const slotCosts = calculateCosts({
        ...
        varnishMlPerPlate: isInImpressionGroup ? 0 : slot.varnishMlPerPlate,
