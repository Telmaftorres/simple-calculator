    ...
    data: {
      ...
      products: validated.products && validated.products.length > 0 ? {
        create: validated.products.map((p) => ({
          ...
          varnishMlPerPlate: p.varnishMlPerPlate ?? 0,
      ...
      data: {
        ...
        products: validated.products?.length ? {
          create: validated.products.map((p) => ({
            ...
            varnishSurfacePercent: p.varnishSurfacePercent ?? 0, varnishMlPerPlate: p.varnishMlPerPlate ?? 0, flatColorSurfacePercent: p.flatColorSurfacePercent ?? 0,
    ...
    include: {
      ...
      productionSheet: {
        include: {
          ...
          productionAchatItems: { orderBy: { position: 'asc' } },
        },
      },
    },
  })
